...
import json
def easyCheck(s):
    return "CopyCtToDensityTableFromAnotherCbctImage" in s
def scoreForDict(d):
    import sys
...
def dictMatchScore_0(d):
    ...
    steps += 1
    score_temp = lib.score_generic(d, "CopyCtToDensityTableFromAnotherCbctImage")
    ...
    if score_temp == sys.maxint:
        ...
        score += score_temp
    steps += 1
    score_temp = lib.score_generic(d.get("value").get("value"), "FromExamination")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
