...
import json
def easyCheck(s):
    return "EditDefaultPattern" in s
def scoreForDict(d):
    import sys
...
def argumentsRight(d):
    args = ["ScheduleString","NumberOfFractionsPerDay"]
    too_many = []
...
def dictMatchScore_0(d):
    ...
    steps += 1
    score_temp = lib.score_generic(d, "EditDefaultPattern")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
    steps += 1
    score_temp = lib.score_generic(d.get("value"), "FractionationPattern")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
