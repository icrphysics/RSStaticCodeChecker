...
import json
def easyCheck(s):
    return "AddReferenceOptimizationFunction" in s
def scoreForDict(d):
    import sys
...
def argumentsRight(d):
    args = ["FunctionType","ReferenceDoseDistribution","RoiName","IsConstraint","RestrictAllBeamsIndividually","RestrictToBeam","IsRobust","RestrictToBeamSet","UseRbeDose"]
    too_many = []
...
def dictMatchScore_0(d):
    ...
    steps += 1
    score_temp = lib.score_generic(d, "AddReferenceOptimizationFunction")
    ...
    if score_temp == sys.maxint:
        ...
        score += score_temp
    steps += 1
    score_temp = score_Plan.get_score(d.get("value").get("value").get("value"))
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
