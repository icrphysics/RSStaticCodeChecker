...
import json
def easyCheck(s):
    return "GetRelativeVolumeAtDoseValues" in s
def scoreForDict(d):
    import sys
...
def dictMatchScore_0(d):
    ...
    steps += 1
    score_temp = lib.score_generic(d, "GetRelativeVolumeAtDoseValues")
    ...
    if score_temp == sys.maxint:
        ...
        score += score_temp
    steps += 1
    score_temp = lib.score_generic(d.get("value").get("value"), "BeamDoses")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
    steps += 1
    score_temp = lib.score_generic(d.get("value").get("value").get("value"), "FractionDose")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
    ...
    if score_temp == sys.maxint:
        ...
        score += score_temp
    steps += 1
    score_temp = lib.score_generic(d.get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value"), "Cases")
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
    steps += 1
    score_temp = score_Patient.get_score(d.get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value").get("value"))
    if score_temp == sys.maxint:
        return sys.maxint
    else:
        score += score_temp
...
def get_info():
    return json.loads("{\"base\": \"Patient.Cases.[].TreatmentPlans.[].BeamSets.[].DrrSettings.[].BeamListSource.FractionDose.BeamDoses.[]\", \"params\": [\"RoiName\", \"DoseValues\"], \"method\": \"GetRelativeVolumeAtDoseValues\", \"description\": \"GetRelativeVolumeAtDoseValues(..)\\r\\n  Method for retrieving DVH points.\\r\\n  Parameters:\\r\\n    RoiName - The name of the ROI.\\r\\n    DoseValues - The dose values in cGy.\\r\\n  Returns:\\r\\n    The relative volumes at the dose values.\\r\\n\"}")
