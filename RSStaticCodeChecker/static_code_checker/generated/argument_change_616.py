...
import json
def easyCheck(s):
    return "SetImageSpecificCtToDensityTable" in s
def scoreForDict(d):
    import sys
...
def dictMatchScore_0(d):
    ...
    steps += 1
    score_temp = lib.score_generic(d, "SetImageSpecificCtToDensityTable")
...
def get_info():
    return json.loads("{\"base\": \"Plan.BeamSets.[].DrrSettings.[].BeamListSource.FractionDose.BeamDoses.[].OnDensity.FromExamination.EquipmentInfo\", \"params\": [\"HU\", \"Rho\"], \"method\": \"SetImageSpecificCtToDensityTable\", \"description\": \"SetImageSpecificCtToDensityTable(..)\\r\\n  Action that sets an image specific CT to density table for an \\r\\n  Examination.\\r\\n  Currently it can only be used for CT images.\\r\\n  Parameters:\\r\\n    HU - Array with CT values [in HU]\\r\\n    Rho - Array with mass density values [in g/cm^3]\\r\\n\"}")
